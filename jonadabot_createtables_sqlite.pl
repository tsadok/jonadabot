...
$q = $db->prepare("CREATE TABLE IF NOT EXISTS alarm (
     ...
     nick       text,
     sender     text,
     setdate    $datetimetype,
     alarmdate  $datetimetype,
     message    text,
     viewed     $datetimetype,
     snoozetill $datetimetype,
     viewcount  integer,
     flags      text)");
$q->execute();
...
$q = $db->prepare("CREATE TABLE IF NOT EXISTS notification(
     id          integer NOT NULL PRIMARY KEY,
     usernick    text,
     enqueued    $datetimetype,
     dequeued    $datetimetype,
     flags       text,
     message     text)");
$q->execute();
...
my @var = (
           ...
           ['pingtimelimit'        => [45, 60, 90, 180], "Number of seconds of no channel activity before pinging"],
